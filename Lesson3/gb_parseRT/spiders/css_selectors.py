...
def get_author_id(resp):
    ...
    for script in resp.css('script'):
        try:
            if marker in script.css('::text').extract_first(): # вытаскиваем имя автора из скрипта с ииспользованием regex101
...
CAR_DATA = {
    ...
    'price': lambda resp: float(
        resp.css('div.AdvertCard_price_3dDCr::text').get().replace('\u2009', '')
    ),
    'photos': lambda resp: [
